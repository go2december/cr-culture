            ...
            <section className="container mx-auto max-w-7xl px-4 pb-24">
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mt-8 animate-fade-in-up delay-400">
                    {sampleActivities.map((activity, i) => (
                        <div key={activity.slug} className={`card-modern bg-white flex flex-col h-full group rounded-3xl overflow-hidden border border-base-200 shadow-sm hover:shadow-[0_8px_30px_rgb(212,175,55,0.08)] hover:border-secondary/50 transition-all duration-400 delay-${(i % 3 + 1) * 100}`}>
                            ...
                            <div className="p-8 flex flex-col flex-grow relative">
                                <span className={`text-[10px] font-bold px-2.5 py-1 rounded inline-block w-fit uppercase tracking-widest mb-4 ${activity.level === 'province' ? 'bg-primary/10 text-primary' : 'bg-secondary/20 text-secondary-dark'}`}>
                                    {activity.level === 'province' ? 'กิจกรรมระดับจังหวัด' : `กิจกรรมระดับอำเภอ${activity.districtName}`}
                                </span>
                                ...
                                <div className="pt-5 border-t border-base-100 flex items-center justify-between mt-auto">
                                    <Link href={`/activities/${activity.slug}`} className="text-sm font-semibold text-primary hover:text-secondary-dark transition-colors flex items-center gap-1 group/link w-full justify-between">
                                        ดูรายละเอียดกิจกรรม
