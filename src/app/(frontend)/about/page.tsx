...
import { getAboutPage, getPageHeroes } from '@/lib/payload'
...
export default async function AboutPage() {
    ...
    const historyText = aboutData?.historyPlain || ''
    const historyParagraphs = historyText
        ? historyText.split('\n').filter((p: string) => p.trim().length > 0)
        : defaults.historyParagraphs
    const hero = pageHeroes?.about || {}
    const heroMedia = hero.heroImage as MediaLike
    const heroImageUrl = resolveMediaUrl(heroMedia)
    const heroImageAlt = resolveMediaAlt(heroMedia, (hero.title as string) || 'ภาพพื้นหลังหน้าเกี่ยวกับเรา')
                        ...
                        <div className="container mx-auto max-w-7xl px-4 relative z-20">
                            <div className="max-w-4xl text-left">
                                <div className="inline-flex items-center gap-2 px-5 py-2 rounded-full text-sm font-medium shadow-sm mb-6 bg-white/14 backdrop-blur-md border border-white/20 text-white reveal-soft">
                                    ...
                                    {(hero.eyebrow as string) || 'ข้อมูลองค์กร'}
                                </div>
                                ...
                                <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold mb-6 tracking-tight font-display text-white leading-[1.05] drop-shadow-lg reveal-soft stagger-1">
                                    {(hero.title as string) || 'เกี่ยวกับเรา'}
                                </h1>
                                ...
                                <p className="text-lg md:text-xl max-w-3xl font-light leading-relaxed text-white/82 reveal-soft stagger-2">
                                    {(hero.subtitle as string) || 'ทำความรู้จักสภาวัฒนธรรมจังหวัดเชียงราย องค์กรแกนนำในการขับเคลื่อน อนุรักษ์ และสืบสานมรดกภูมิปัญญาล้านนา'}
                                </p>
                            </div>
                        </div>
                    </>
                ...
                <section className="mb-24 relative accent-panel">
                    <div className="bg-white rounded-3xl p-8 md:p-12 lg:p-16 shadow-[0_8px_30px_rgb(212,175,55,0.05)] border border-base-200">
                        ...
                        <div className="prose prose-lg max-w-3xl mx-auto prose-p:text-base-content/70 prose-p:font-light prose-p:leading-relaxed prose-headings:text-primary relative z-10">
                            {historyParagraphs.map((paragraph: string, i: number) => (
                                <p key={i} className={i === 0 ? 'first-letter:text-6xl first-letter:font-bold first-letter:text-primary first-letter:mr-2 first-letter:float-left first-letter:leading-none first-letter:font-display' : ''}>
                                    {paragraph}
                                </p>
                            ))}
                        </div>
