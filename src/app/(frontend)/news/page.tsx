        ...
        <div className="bg-slate-50 min-h-screen font-sans">
            ...
            <div className="container mx-auto max-w-5xl px-4 py-8 animate-fade-in-up delay-300">
                <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                    <Link href="/news" className={`px-6 py-2.5 rounded-full text-sm font-medium shadow-sm transition-all ${selectedType === 'all' ? 'bg-primary text-white shadow-primary/20 hover:bg-primary-dark' : 'bg-white border border-base-200 text-base-content/70 hover:border-primary hover:text-primary'}`}>
                        ทั้งหมด
                    </Link>
                    <Link href="/news?type=general" className={`px-5 py-2.5 rounded-full border text-sm font-medium transition-all shadow-sm hover:shadow-md flex items-center gap-2 ${selectedType === 'general' ? 'bg-primary text-white border-primary' : 'bg-white border-base-200 text-base-content/70 hover:border-primary hover:text-primary'}`}>
                        <div className={selectedType === 'general' ? 'text-white' : typeConfig.general.color.split(' ')[0]}>{typeConfig.general.icon}</div> ข่าวทั่วไป
                    </Link>
                    <Link href="/news?type=video" className={`px-5 py-2.5 rounded-full border text-sm font-medium transition-all shadow-sm hover:shadow-md flex items-center gap-2 ${selectedType === 'video' ? 'bg-secondary-dark text-white border-secondary-dark' : 'bg-white border-base-200 text-base-content/70 hover:border-secondary hover:text-secondary-dark'}`}>
                        <div className={selectedType === 'video' ? 'text-white' : typeConfig.video.color.split(' ')[0]}>{typeConfig.video.icon}</div> วิดีโอ
                    </Link>
                    <Link href="/news?type=document" className={`px-5 py-2.5 rounded-full border text-sm font-medium transition-all shadow-sm hover:shadow-md flex items-center gap-2 ${selectedType === 'document' ? 'bg-sky-600 text-white border-sky-600' : 'bg-white border-base-200 text-base-content/70 hover:border-sky-500 hover:text-sky-700'}`}>
            ...
            <section className="container mx-auto max-w-5xl px-4 pb-24 relative z-20">
                <div className="space-y-6 mt-8">
                    {newsItems.length > 0 ? newsItems.map((news: any, i: number) => {
                        const config = typeConfig[news.type as keyof typeof typeConfig] || typeConfig.general

                        return (
                            <article key={news.slug || news.id} className={`group bg-white rounded-3xl border border-base-200 shadow-[0_8px_30px_rgb(212,175,55,0.05)] hover:shadow-xl hover:border-secondary/30 transition-all duration-400 overflow-hidden animate-fade-in-up delay-${(i % 4 + 1) * 100}`}>
                                    ...
                                    <div className="p-8 md:p-10 flex-1 flex flex-col justify-center relative z-10 w-full sm:w-auto">
                                        <div className="flex items-center flex-wrap gap-3 mb-4">
                                            ...
                                            <span className="text-sm font-medium text-base-content/50 flex items-center gap-1.5">
                                                ...
                                                {new Date(news.date || news.createdAt).toLocaleDateString('th-TH', {
                                                    year: 'numeric',
                                                    month: 'long',
                                                    day: 'numeric',
                                                })}
                ...
                <div className="flex justify-center mt-16 animate-fade-in-up delay-500">
                    ...
                    <div className="inline-flex items-center justify-center p-1.5 bg-white rounded-full border border-base-200 shadow-sm">
                        <Link href={`?type=${selectedType}&page=${currentPage - 1}`} className={`w-10 h-10 flex items-center justify-center rounded-full ${hasPrevPage ? 'text-base-content/70 hover:bg-secondary/10 hover:text-secondary-dark transition-colors' : 'text-base-content/40 cursor-not-allowed pointer-events-none'}`}>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
                        </Link>
                        
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                            <Link key={page} href={`?type=${selectedType}&page=${page}`} className={`w-10 h-10 flex items-center justify-center rounded-full font-medium ${currentPage === page ? 'bg-gradient-to-br from-primary to-primary-light text-white shadow-sm' : 'text-base-content/70 hover:bg-secondary/10 hover:text-secondary-dark transition-colors'}`}>{page}</Link>
                        ))}

                        <Link href={`?type=${selectedType}&page=${currentPage + 1}`} className={`w-10 h-10 flex items-center justify-center rounded-full ${hasNextPage ? 'text-base-content/70 hover:bg-secondary/10 hover:text-secondary-dark transition-colors' : 'text-base-content/40 cursor-not-allowed pointer-events-none'}`}>
