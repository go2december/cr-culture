...
export default function ActiveFilters({ tags }: { tags?: TagLike[] }) {
    ...
    if (tag) {
        ...
        const label = found ? `🏷️ #${found.name}` : `🏷️ #${tag}`
        filters.push({ type: 'tag', label, value: tag })
    }
    ...
    return (
        <div className="flex items-center gap-2 flex-wrap mb-6 p-4 bg-primary/5 rounded-2xl border border-primary/10">
            <span className="text-sm text-base-content/60 font-medium">ฟิลเตอร์:</span>

            {filters.map((filter) => (
                <span
                    key={filter.type}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white text-primary rounded-full text-sm border border-primary/20 shadow-sm hover:border-primary/40 transition-colors"
                >
                    {filter.label}
                    <Link
            ...
            <Link href="/heritage" className="text-sm text-primary hover:text-primary-dark font-medium hover:underline ml-auto">
                ล้างทั้งหมด
            </Link>
        </div>
    )
