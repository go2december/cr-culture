...
## Recent Work (ปรับปรุงแหล่งที่มาของข้อมูลกรรมการจังหวัด — 1 กรกฎาคม 2569)
- **CMS Schema & Fields**: ปรับปรุงคอลเลกชัน `provincial-board` โดยเพิ่มฟิลด์ `sourceType` (ระบุว่าป้อนเองหรือดึงข้อมูลอัตโนมัติ) และฟิลด์ `district` (เชื่อมโยงอำเภอ) พร้อมระบบซ่อน/แสดงและตรวจสอบข้อมูลแบบมีเงื่อนไขตามแหล่งข้อมูลที่เลือก
- **Data Mapping & Types**: ขยายอินเทอร์เฟซ `PublicBoardMember` และปรับปรุงฟังก์ชัน `mapBoardMember` ให้ส่งต่อฟิลด์ `sourceType` และ `district` ที่ถูกแปลงความสัมพันธ์ครบถ้วนไปยังฟรอนต์เอนด์
- **Board Page Update**: ปรับปรุงหน้าคณะกรรมการจังหวัด `/about/board` ให้แสดงรายการกรรมการ (ตำแหน่งระดับ 4) จากตารางกรรมการจังหวัดโดยตรง ซึ่งสามารถเลือกป้อนข้อมูลเองหรือให้ระบบค้นหาและดึงรายละเอียดชื่อ/รูปภาพ/ลิงก์ของประธานสภาวัฒนธรรมอำเภอนั้นๆ มาแสดงผลโดยอัตโนมัติ
- **Verification**: ผ่านการตรวจสอบคุณภาพ 100% ทั้งการตรวจโค้ด `npm run lint`, การเช็กความปลอดภัยชนิดข้อมูล `npm run typecheck`, และบิลด์โปรดักชัน `npm run build`
